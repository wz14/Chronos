...
func NewVACS(pid *idchannel.PrimitiveID, s config.Start, q func(message *pb.Message) bool) (ACS, error) {
	conf := s.GetConfig()
	b := &VACS{
		rootpid:    pid,
		nig:        s.Getnig(),
...
func (b *VACS) Decided(message *pb.Message) ([]*pb.Message, error) {
	go b.rbcb(message)
	go b.rbcd()
	go b.aba()
	l := b.rollback()
...
func (b *VACS) rbcb(message *pb.Message) {
...
func (b *VACS) rbcd() {
	for i := 0; i < b.c.N; i++ {
		go func(j int) {
			...
			if b.q(tmp) {
				b.l.Infof("satisfy predicate")
				b.rbc1Notify <- j
				b.rbcDeliver[j] <- mes
			} else {
				b.l.Infof("unsatisfy predicate")
			}
		}(i)
	}
}
