...
func NewBenorACS(pid *idchannel.PrimitiveID, s config.Start) (ACS, error) {
	conf := s.GetConfig()
	b := &BenorACS{
		rootpid:    pid,
		nig:        s.Getnig(),
...
func (b *BenorACS) Decided(message *pb.Message) ([]*pb.Message, error) {
	go b.rbcb(message)
	go b.rbcd()
	go b.aba()
	l := b.rollback()
...
func (b *BenorACS) rbcb(message *pb.Message) {
